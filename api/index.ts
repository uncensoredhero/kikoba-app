...
export default function handler(req: IncomingMessage & { url?: string }, res: ServerResponse) {
  ...
  if (req.url === "/" || req.url === "/api/index") {
    try {
      ...
      const phaseC = readFileSync(join(process.cwd(), "public", "phase-c-polish.js"), "utf8");
      const phaseCSocial = readFileSync(join(process.cwd(), "public", "phase-c-social.js"), "utf8");
      ...
      html = html.replace("</body>", `<script>${enhancement}</script><script>${createChama}</script><script>${phaseB}</script><script>${phaseC}</script><script>${phaseCSocial}</script></body>`);
      res.statusCode = 200;
